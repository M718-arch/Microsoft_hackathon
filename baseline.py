...
def detect_aspects(text: str, star_rating: int) -> list[str]:
    text_l = text.lower()
    found  = []
    ...
    if not found:
        # Very short or no keywords — use star rating to infer
        return ["general"]
    return found


def per_aspect_sentiment(text: str, aspects: list[str], overall_sent: str) -> dict:
    """
    Try to assign sentiments per aspect using local context windows.
    Falls back to the overall sentiment.
    """
    text_l = text.lower()
    result = {}
...
def predict_row(row) -> dict:
    ...
    overall = detect_sentiment(text)
    sents   = per_aspect_sentiment(text, aspects, overall)
